...
public class Cartridge implements AddressSpace {
    ...
    public Cartridge(GameboyOptions options, File saveFile) throws IOException {
        ...
        if (type.isBattery() && options.isSupportBatterySaves()) {
            battery = new FileBattery(saveFile, FilenameUtils.removeExtension(saveFile.getName()), 0x2000 * romBanks);
        } else {
